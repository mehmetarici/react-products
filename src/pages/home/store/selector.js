export function getProducts(state) {
    const {products, searchText, sortBy} = state;
    if (searchText) {
        return products
            .filter(product =>
    ...
    if (sortBy)
        return _sortProducts(products, sortBy)
...
function _sortProducts(products, sortBy) {
    ...
    if (sortBy === "minAmount")
        result.sort((p1, p2) => p1.amount.discounted - p2.amount.discounted);
    else if (sortBy === "maxAmount")
        result.sort((p1, p2) => p2.amount.discounted - p1.amount.discounted);
    else if (sortBy === "aToZ")
        result.sort((p1, p2) => {
